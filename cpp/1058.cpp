...
public:
	...
	void getPtr(string,int);
	void setPtr(string, int);
...
void Memory::printMemoryValue(string type, int index) {
	...
	//int
	else if (type == "int" || type == "char*" || type == "short*" || type == "int*" || type == "String*") {
		if (index + 4 > memoryArray.size()) {
			cout << "Violation Access." << endl;
			return;
		}
...
void Memory::getPtr(string type, int index) {
	if (index + 4 > memoryArray.size()) {
		cout << "Violation Access." << endl;
		return;
	}
	...
	if (memoryArray[index + 3] > 127) {//負數
		for (int i = 0; i < 4; i++) {
			printPtr -= (255 - memoryArray[index + i]) * pow(256, i);
		}
		printPtr--;
	}
	else {
		for (int i = 0; i < 4; i++) {
			printPtr += memoryArray[index + i] * pow(256, i);
		}
	}
	printMemoryValue(type, printPtr);
}

void Memory::setPtr(string type, int index) {
	if (index + 4 > memoryArray.size()) {
		cout << "Violation Access." << endl;
		return;
	}
	...
	while (cin >> N >> k) {
		...
		while (k--) {
			...
			if (operation == "Set") {
				...
				else if (type == "char*" || type == "short*" || type == "int*" || type == "String*")
					memory.typeInt(address);
			}
			else if (operation == "SetPtr") {
					memory.setPtr(type,address);
			}
			else if (operation == "Get") {
				memory.printMemoryValue(type, address);
			}
			else if (operation == "GetPtr") {
				memory.getPtr(type, address);
			}
		}
	}
	return 0;
